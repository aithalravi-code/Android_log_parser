 ...
 * @param {number} year - Optional year to use (defaults to current year)
 * @returns {Date|null} - Parsed date or null if invalid
 *
 * @example
 * logcatToDate('07-02 09:33:33.365', 2025) // Returns Date object for July 2nd, 2025, 9:33:33.365
 */
export function logcatToDate(logcatTimestamp, year) {
    // logcatTimestamp is in "MM-DD HH:mm:ss.SSS" format, e.g., "07-02 09:33:33.365"
    if (!logcatTimestamp || logcatTimestamp.length < 18) {
        return null; // Invalid format
    }
    // Use provided year or fall back to current year for backwards compatibility
    ...
    const date = new Date(targetYear, month, day, hours, minutes, seconds, milliseconds);
